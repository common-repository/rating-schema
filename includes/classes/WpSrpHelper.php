...
if ( ! class_exists( 'WpSrpSettings' ) ):
	...
	class WpSrpHelper {
		...
		function nonceText() {
			return "wpsrp_nonce_secret_text";
		}
		...
		function fix1_2DataMigration(){
			global $WpSrpSchema;
			$fix_1_2 = get_option($WpSrpSchema->options['1_2_fix']);
			$installed_version = get_option($WpSrpSchema->options['installed_version']);
			if(empty($fix_1_2) && empty($installed_version)){
				...
				if ( ! empty( $ids ) ) {
					...
					foreach ( $ids as $id ) {
						foreach ( $schemaFields as $schemaID => $schema ) {
							...
							$metaData     = array();
							if ( ! empty( $getRawMeta ) ) {
								if ( $WpSrpSchema->isValidBase64( $getRawMeta ) ) {
				...
				update_option($WpSrpSchema->options['1_2_fix'], true);
			}
		}
	}
